...
int resetDeviceConnection(int vid, int pid){
    /*Open libusb*/
    int resetStatus = 0;
...
/*
	...
	if (erase_eeprom == 0) {
		printf("DGB: Encode new eeprom image.\r\n");
	...
	if (memcmp(old, new, len) == 0) {
		...
		printf("Write content to eeprom\n");
		ee_write(new, len);
...
*/	
	if(reset_device == 1){
		...
		retValue = resetDeviceConnection(ee.old_vid, ee.old_pid);
		printf("RetVale=%d\n",retValue);
		if(retValue != 0){
			fprintf(stderr, "Reseting USB device vid_%04x&pid_%04x failed!\n",ee.old_vid,ee.old_pid);
